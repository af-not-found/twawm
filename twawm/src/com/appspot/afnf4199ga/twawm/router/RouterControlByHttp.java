...
public class RouterControlByHttp {
	...
	protected static final String NOT_SITE_LOCAL_ADDR = "NOT_SITE_LOCAL_ADDR";
	...
	public static int exec(Context context, CTRL ctrl, RouterInfo routerInfo) {
		...
		try {
			// ルーターIPアドレスを取得
			String routerIpAddr = getRouterIpAddr(new InetLookupWrappter(), Const.getPrefApIpAddr(context));
			if (routerIpAddr == null) {
				return 11;
			}
			// ルーターIPアドレスがPrivateIPでない
			else if (routerIpAddr == NOT_SITE_LOCAL_ADDR) {
	...
	private static void discardContent(HttpResponse response) {
		try {
			if (response != null) {
				HttpEntity entity = response.getEntity();
				if (entity != null) {
